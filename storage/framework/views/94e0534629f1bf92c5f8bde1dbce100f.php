...
<head>
    ...
    <title>Pack <?php echo e($bundle->name); ?> - Success Business Training</title>
    <meta name="description" content="<?php echo e(Str::limit($bundle->description, 160)); ?>">
    ...
    <main class="content-container">
        ...
        <article>
            <div class="details-card">
                ...
                <h2 class="section-title" style="margin-top: 3rem; margin-bottom: 2rem;">Formations incluses dans ce pack (<?php echo e($bundle->trainings->count()); ?>)</h2>
                <div>
                    <?php $__currentLoopData = $formattedTrainings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trainingItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="bundle-training-item">
                            <div class="training-img-box" style="background-image: url('<?php echo e($trainingItem['illustration']); ?>');"></div>
                            <div class="training-content-box">
                                <div>
                                    <a href="<?php echo e(route('training.show', $trainingItem['id'])); ?>" class="training-title-link" target="_blank">
                                        <?php echo e($trainingItem['name']); ?> ↗
                                    </a>
                                    <p style="font-size: 0.9rem; color: #64748b; margin: 0.25rem 0 0.5rem 0;">
                                        📅 Début : <strong><?php echo e($trainingItem['date']); ?></strong> | 📍 Lieu : <strong><?php echo e($trainingItem['location']); ?></strong>
                                    </p>
                                    <p style="font-size: 0.95rem; margin: 0.5rem 0; line-height: 1.5; color: var(--text-main);">
                                        <?php echo e(Str::limit($trainingItem['description'], 180)); ?>

                                    </p>
                                </div>
                                <div class="skills-grid">
                                    <?php $__currentLoopData = $trainingItem['skills']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <span class="skill-pill" style="background-color: <?php echo e($skill->badge_color); ?>;">
        ...
        <aside class="sidebar-box">
            <div class="pricing-card">
                ...
                <div class="price-badge-row">
                    ...
                    <strong class="current-price"><?php echo e(number_format($bundle->price, 0, ',', ' ')); ?> CFA</strong>
                    <span class="old-price">Cumulé : <?php echo e(number_format($totalPromoPrice, 0, ',', ' ')); ?> CFA</span>
                    <?php if($savings > 0): ?>
                        <div class="savings-badge">
                            ✨ Économisez <?php echo e(number_format($savings, 0, ',', ' ')); ?> CFA !
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <ul class="meta-list">
                    <li class="meta-item">
                        ...
                        <span><?php echo e($bundle->trainings->count()); ?> formations</span>
                    </li>
                    <li class="meta-item">
                        <span class="meta-icon">🔒</span>
                        <span class="meta-label">Accès ressources</span>
                        <span>Inclus (les <?php echo e($bundle->trainings->count()); ?> formations)</span>
                    </li>
                    <li class="meta-item">
                        <span class="meta-icon">📜</span>
                ...
                <?php if(request('status') === 'success'): ?>
                    <div class="thank-you-card" style="text-align: center; padding: 2rem; background: #ffffff; border-radius: 1.5rem; border: 2px solid #10b981; box-shadow: 0 20px 40px rgba(16, 185, 129, 0.08); width: 100%; box-sizing: border-box;">
                        ...
                        <p style="font-size: 0.95rem; line-height: 1.5; color: var(--text-main); margin-bottom: 1.5rem;">
                            Votre demande pour le pack <strong><?php echo e(request('course')); ?></strong> a bien été enregistrée.
                        </p>
                ...
                <?php else: ?>
                    ...
                    <form action="<?php echo e(route('register')); ?>" method="POST" id="main_registration_form">
                        ...
                        <input type="hidden" name="redirect_to" value="<?php echo e(route('bundle.show', $bundle->id)); ?>">
                        <!-- Preselected course -->
                        <input type="hidden" name="course" value="<?php echo e($bundle->name); ?>" id="main_course_input">
                        <!-- Hidden bundle id input -->
                        <input type="hidden" name="bundle_id" value="<?php echo e($bundle->id); ?>" id="main_bundle_id">

                        <div class="form-group-custom">
                            <label>Pack sélectionné</label>
                            <input type="text" class="form-control-custom" value="Pack <?php echo e($bundle->name); ?>" id="selected_course_label" disabled style="font-weight: 700; background-color: #f1f5f9;">
                        </div>
                        ...
                        <?php if(auth()->guard('client')->check()): ?>
                            <div class="alert-premium alert-premium-success" style="margin-top: 1rem; margin-bottom: 1.5rem; background-color: #ecfdf5; border: 1px solid #a7f3d0; color: #065f46; padding: 1rem; border-radius: 1rem;">
                                Bonjour <strong><?php echo e(auth('client')->user()->name); ?></strong> ! Vous êtes connecté(e). Vos coordonnées sont pré-remplies automatiquement.
                            </div>
                            <input type="hidden" name="name" value="<?php echo e(auth('client')->user()->name); ?>">
                            <input type="hidden" name="phone" value="<?php echo e(auth('client')->user()->phone); ?>">
                            <input type="hidden" name="email" value="<?php echo e(auth('client')->user()->email); ?>">
                        <?php else: ?>
                            <div class="form-group-custom">
...
<?php /**PATH C:\xampp\htdocs\formation\resources\views/bundle_details.blade.php ENDPATH**/ ?>